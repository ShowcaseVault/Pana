...
const RecordingList = ({ refreshTrigger }) => {
  ...
  if (recordings.length === 0) {
       return <div className="empty-state">No recordings yet. Start speaking!</div>
  }
          ...
          <div key={rec.id} className="recording-card">
            ...
            <div className="card-info">
                <div className="info-row main">
                    <span className="date">{new Date(rec.recorded_at).toLocaleDateString()}</span>
                    <span className="time">{new Date(rec.recorded_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                </div>
                <div className="info-row sub">
                    <span className="duration">
                        <Clock size={12} /> {Math.floor(rec.duration_seconds / 60)}:{(rec.duration_seconds % 60).toString().padStart(2, '0')}
                    </span>
                    {rec.location_text && (
