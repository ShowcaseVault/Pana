...
async def create_transcription_endpoint(
    ...
    if new_transcription is False:
        raise HTTPException(
            status_code=400, detail="Transcription already exists for this recording"
        )
...
async def get_transcriptions_endpoint(
    ...
    return SuccessResponse(data=transcriptions, message="Transcriptions retrieved successfully")


@router.get("/{transcription_id}", response_model=SuccessResponse | FailureResponse)
...
# @router.patch("/{transcription_id}", response_model=SuccessResponse | FailureResponse)
...
# @router.delete("/{transcription_id}", response_model=SuccessResponse | FailureResponse)
